...
["", "biosoftnamegen.dart", , O, {
  "^": "",
  main: [function() {
    var t1, t2, t3;
    t1 = W.HttpRequest_getString("names.json", null, null).then$1(O.NameGen_parseNames$closure()).then$1(new O.main_closure());
    t2 = $.Zone__current;
    t3 = P._registerErrorHandler(new O.main_closure0(), t2);
    t2.toString;
    t1._addListener$1(H.setRuntimeTypeInfo(new P._Future(0, t2, null, null, null, null, t3, null), [null]));
  }, "call$0", "main$closure", 0, 0, 0],
  addNewBioSoftName: [function($event) {
    var namePair, newAbbrev, newName;
    namePair = $.get$nameGen().getNamePair$0();
    newAbbrev = namePair[0];
    newName = namePair[1];
    document.querySelector("#abbreviation").textContent = newAbbrev;
    document.querySelector("#name").textContent = newName;
  }, "call$1", "addNewBioSoftName$closure", 2, 0, 1],
  main_closure: {
    "^": "Closure:10;",
    call$1: function(_) {
      var t1;
      document.querySelector("#abbreviation").textContent = "Click for inspiration.";
      t1 = J.get$onClick$x(document.querySelector("#container"));
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(O.addNewBioSoftName$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
    }
  },
  main_closure0: {
    "^": "Closure:10;",
    call$1: function(oops) {
      P.print("error reading names from json");
      P.print(oops);
      document.querySelector("#name").textContent = "Error reading JSON; try again later.";
    }
  },
  NameGen: {
    "^": "Object;",
    getNamePair$0: function() {
      var adjPhraseList, t1, adjPhrase, adjPhraseAbbrev, domainList, domain, domainAbbrev, actionList, action, actionAbbrev, doerList, doer, doerAbbrev, abbrev, $name;
      if ($.get$NameGen_rng().nextDouble$0() > 0.5) {
        adjPhraseList = this.randomListFromList$1($.NameGen_adjectivePhrases);
        t1 = J.getInterceptor$asx(adjPhraseList);
        adjPhrase = J.$add$ns(t1.$index(adjPhraseList, 0), " ");
        adjPhraseAbbrev = this.randomStringFromList$1(t1.$index(adjPhraseList, 1));
      } else {
        adjPhrase = "";
        adjPhraseAbbrev = "";
      }
      domainList = this.randomListFromList$1($.NameGen_domains);
      t1 = J.getInterceptor$asx(domainList);
      domain = J.$add$ns(t1.$index(domainList, 0), " ");
      domainAbbrev = this.randomStringFromList$1(t1.$index(domainList, 1));
      if ($.get$NameGen_rng().nextDouble$0() > 0.5) {
        actionList = this.randomListFromList$1($.NameGen_actions);
        t1 = J.getInterceptor$asx(actionList);
        action = J.$add$ns(t1.$index(actionList, 0), " ");
        actionAbbrev = this.randomStringFromList$1(t1.$index(actionList, 1));
      } else {
        action = "";
        actionAbbrev = "";
      }
      doerList = this.randomListFromList$1($.NameGen_doers);
      t1 = J.getInterceptor$asx(doerList);
      doer = t1.$index(doerList, 0);
      doerAbbrev = this.randomStringFromList$1(t1.$index(doerList, 1));
      abbrev = J.$add$ns(J.$add$ns(J.$add$ns(adjPhraseAbbrev, domainAbbrev), actionAbbrev), doerAbbrev);
      $name = J.$add$ns(J.$add$ns(J.$add$ns(adjPhrase, domain), action), doer);
      $name = this.startsWithAVowel$1($name) ? C.JSString_methods.$add("An ", $name) : C.JSString_methods.$add("A ", $name);
      if (J.get$length$asx(abbrev) < 3)
        return this.getNamePair$0();
      else
        return [abbrev, $name];
    },
    randomListFromList$1: function(myList) {
      var t1 = J.getInterceptor$asx(myList);
      return t1.$index(myList, $.get$NameGen_rng().nextInt$1(t1.get$length(myList)));
    },
    randomStringFromList$1: function(myList) {
      var t1 = J.getInterceptor$asx(myList);
      return t1.$index(myList, $.get$NameGen_rng().nextInt$1(t1.get$length(myList)));
    },
    startsWithAVowel$1: function(word) {
      if (C.JSString_methods.contains$1("AEIOUaeiou", J.$index$asx(word, 0)))
        return true;
      else
        return false;
    },
    ...
    static: {"^": "NameGen_rng,NameGen_adjectivePhrases,NameGen_domains,NameGen_actions,NameGen_doers", NameGen_parseNames: [function(namestring) {
        var names, t1;
        names = C.JsonCodec_null_null.decode$1(namestring);
        t1 = J.getInterceptor$asx(names);
        $.NameGen_adjectivePhrases = t1.$index(names, "adjective phrases");
        $.NameGen_domains = t1.$index(names, "domains");
        $.NameGen_actions = t1.$index(names, "actions");
        $.NameGen_doers = t1.$index(names, "doers");
      }, "call$1", "NameGen_parseNames$closure", 2, 0, 2]}
  }
},
1],
...
["dart.async", "dart:async", , P, {
  ...
  _AsyncCompleter: {
    ...
    }, "completeError$1", "call$2", "call$1", "get$completeError", 2, 2, 12, 13]
  },
  _Future: {
    ...
    }, "_completeError$1", "call$2", "call$1", "get$_completeError", 2, 2, 14, 13],
...
$.NameGen_adjectivePhrases = null;
$.NameGen_domains = null;
$.NameGen_actions = null;
$.NameGen_doers = null;
$.printToZone = null;
$._nextCallback = null;
$._lastCallback = null;
...
Isolate.$lazy($, "nameGen", "nameGen", "get$nameGen", function() {
  var t1 = new O.NameGen();
  t1.NameGen$0();
  return t1;
});
Isolate.$lazy($, "rng", "NameGen_rng", "get$NameGen_rng", function() {
...
function dart_precompiled($collectedClasses) {
  ...
  NameGen.builtin$cls = "NameGen";
  if (!"name" in NameGen)
    NameGen.name = "NameGen";
  $desc = $collectedClasses.NameGen;
  if ($desc instanceof Array)
    $desc = $desc[1];
  NameGen.prototype = $desc;
